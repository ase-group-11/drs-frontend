...
// Added: Traffic Override button → navigates to RerouteOverrideScreen
// ═══════════════════════════════════════════════════════════════════════════
...
export const DisasterDetailScreen: React.FC = () => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (deps.status === 'fulfilled') {
        ...
        setDeploySummary(deps.value?.summary ?? null);
      }
    } catch {}
    setLoading(false);
  };
  ...
  // Determine whether to show the override button.
  return (
    <SafeAreaView style={S.safe} edges={['top', 'left', 'right']}>
      <StatusBar barStyle="light-content" backgroundColor={RED} />
      ...
      <View style={S.header}>
        ...
        <TouchableOpacity style={S.hBtn} onPress={loadAll}>
        ...
        {(['details', 'deployments'] as const).map(t => (
          ...
          >
            <Text style={[S.tabTxt, tab === t && S.tabTxtOn]}>
              {t === 'details' ? 'Details' : `Units (${deployments.length})`}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        <ScrollView contentContainerStyle={{ padding: spacing.md }}>
          ...
          <View style={[S.card, { borderLeftWidth: 4, borderLeftColor: col }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
              <Text style={{ fontSize: 32, lineHeight: 42, marginRight: 10 }}>
                {TYPE_EMOJI[d.type] ?? '⚠️'}
              </Text>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 18, lineHeight: 24, fontWeight: '700', color: '#1F2937' }}>
                  {(d.type ?? '').replace(/_/g, ' ')}
                </Text>
                <View style={{ flexDirection: 'row', gap: 6, marginTop: 4 }}>
                  ...
                  <View style={[S.pill, { backgroundColor: (DEP_COLOR[d.disaster_status] ?? '#6B7280') + '20' }]}>
                    <Text style={[S.pillTxt, { color: DEP_COLOR[d.disaster_status] ?? '#6B7280' }]}>
                      {d.disaster_status}
                    </Text>
                  </View>
                </View>
          ...
          {/* Location */}
          <View style={S.card}>
            <Text style={S.ct}>Location</Text>
            <R l="Address" v={d.location_address ?? '—'} />
            {d.location && (
              <R l="Coordinates" v={`${d.location.lat?.toFixed(4)}, ${d.location.lon?.toFixed(4)}`} />
            )}
            {d.affected_area && <R l="Affected area" v={d.affected_area} />}
          </View>

          {/* Impact */}
          <View style={S.card}>
            <Text style={S.ct}>Impact</Text>
            <R l="People affected"   v={`${d.people_affected ?? 0}`} />
            <R l="Casualties"        v={d.multiple_casualties ? 'Yes' : 'No'} />
            <R l="Structural damage" v={d.structural_damage   ? 'Yes' : 'No'} />
            <R l="Road blocked"      v={d.road_blocked        ? 'Yes' : 'No'} />
            {d.description && <R l="Description" v={d.description} />}
          </View>

          {/* Assignment */}
          {d.assignment && (
            <View style={S.card}>
              <Text style={S.ct}>Assignment</Text>
              <R l="Department"  v={d.assignment.assigned_department ?? '—'} />
              <R l="Assigned to" v={d.assignment.assigned_to_name    ?? '—'} />
              {d.assignment.assigned_to_phone && (
                <R l="Phone" v={d.assignment.assigned_to_phone} />
              )}
              <R l="Units"   v={`${d.units_assigned ?? 0}`} />
              <R l="Reports" v={`${d.report_count   ?? 0}`} />
            </View>
          )}
          ...
          {d.timeline && (
            <View style={S.card}>
              <Text style={S.ct}>Timeline</Text>
              <R l="Reported"  v={fmt(d.timeline.created_at)}    />
              <R l="Response"  v={fmt(d.timeline.response_time)} />
              <R l="Resolved"  v={fmt(d.timeline.resolved_time)} />
              {d.resolution_notes && <R l="Resolution" v={d.resolution_notes} />}
            </View>
          )}
        ...
        <ScrollView contentContainerStyle={{ padding: spacing.md }}>
          ...
          {deployments.length === 0 ? (
            <View style={{ alignItems: 'center', paddingTop: 60 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '600', color: '#6B7280', marginTop: 8 }}>
                No units deployed
              </Text>
            </View>
          ) : (
            deployments.map((dep, i) => {
              ...
              return (
                <View key={dep.deployment_id ?? i} style={S.card}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    <View style={[S.depBadge, { backgroundColor: sc }]}>
                  ...
                  <Text style={{ fontSize: 15, fontWeight: '700', color: '#1F2937' }}>
                    {u.unit_name || u.unit_code || `Unit #${i + 1}`}
                  </Text>
                  <Text style={{ fontSize: 12, color: '#6B7280', marginTop: 2 }}>
                    {u.department ?? ''} · {u.unit_type ?? ''}
                  </Text>
                  ...
                  {(dep.casualties?.minor > 0 || dep.casualties?.serious > 0) && (
                    <Text style={{ fontSize: 12, color: '#EF4444', marginTop: 4 }}>
                      Casualties: {dep.casualties.minor} minor, {dep.casualties.serious} serious
                    </Text>
                  )}
                </View>
              );
...
const R: React.FC<{ l: string; v: string }> = ({ l, v }) => (
  <Text style={{ fontSize: 13, color: '#6B7280', marginBottom: 4 }}>
    {l}: <Text style={{ color: '#1F2937', fontWeight: '600' }}>{v}</Text>
  </Text>
);

const SB: React.FC<{ label: string; value: string; color: string }> = ({ label, value, color }) => (
  <View style={{
    flex: 1, alignItems: 'center',
    backgroundColor: color + '10', borderRadius: 8, padding: 8,
  }}>
    <Text style={{ fontSize: 18, lineHeight: 24, fontWeight: '800', color }}>{value}</Text>
