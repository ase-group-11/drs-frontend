...
//
// Fixes:
//   1. Removed inner KeyboardAvoidingView + ScrollView — AuthTemplate already
//      provides both, nesting them caused blank space and scroll jitter.
//   2. Added textContentType="newPassword" + autoComplete="new-password" to
//      password fields to suppress iOS/Android autofill overlay (yellow highlight).
//   3. Added importantForAutofill="no" on Android for same reason.
// ═══════════════════════════════════════════════════════════════════════════
...
const OTP_ACCESSORY_ID = 'responder-signup-otp-accessory';
...
interface DropdownProps {
  ...
  items: DropdownItem[]; onSelect: (v: string) => void;
}
          ...
          <View style={DD.sheet}>
            ...
            {items.map(item => (
              ...
              >
                ...
                {item.value === value && <Text style={{ color: '#DC2626', fontSize: 16 }}>✓</Text>}
              </TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
...
const DD = StyleSheet.create({
  ...
  trigger:         {
    ...
    paddingHorizontal: spacing.md, paddingVertical: 14, backgroundColor: '#fff',
  },
...
export const ResponderSignupScreen: React.FC = () => {
  ...
  const validateStep1 = (): string | null => {
    ...
    if (!phone.startsWith('+'))
                            return 'Phone must be in international format (e.g. +353...)';
  ...
  const handleStep1 = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(typeof raw === 'string' && raw.includes('validation error')
  ...
  const handleResendOtp = async (): Promise<void> => {
    ...
    const res = await fetch(`${API_BASE_URL}${API.responder.register()}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        phone_number: phone.trim(), full_name: fullName.trim(),
  ...
  // ── Render ────────────────────────────────────────────────────────────
  // AuthTemplate already provides SafeAreaView + KeyboardAvoidingView + ScrollView.
  // Do NOT nest another KAV/ScrollView inside — causes blank space & jitter.
  return (
    <AuthTemplate header={<AuthHeader title={stepTitle} subtitle={stepSubtitle} />}>
      ...
      <View style={S.progressRow}>
        {[1, 2].map(n => (
          <View key={n} style={[S.progressSeg, step >= n && S.progressSegActive]} />
        ))}
      </View>
      ...
      {step === 1 && (
        <>
          ...
          {/* Password — textContentType="newPassword" suppresses autofill overlay */}
          <Input
            label="Password"
          ...
          <View style={{ marginTop: spacing.md }}>
            {/* Confirm Password — same autofill suppression */}
            <Input
...
const S = StyleSheet.create({
  badge: {
    alignSelf: 'center', backgroundColor: '#FEF2F2', borderRadius: 20,
  ...
  progressSegActive: { backgroundColor: '#DC2626' },
  progressLabel: { fontSize: 12, color: '#64748B', fontWeight: '500', marginBottom: spacing.lg },

  errorBox: {
    backgroundColor: '#FEE2E2', borderRadius: 10, padding: spacing.md,
    marginBottom: spacing.md, borderLeftWidth: 3, borderLeftColor: '#DC2626',
  },
  errorText: { color: '#991B1B', fontSize: 14 },

  strengthRow: { flexDirection: 'row', alignItems: 'center', marginTop: 6, marginBottom: 2, gap: 8 },
  ...
  backRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: spacing.lg, gap: spacing.xs },
